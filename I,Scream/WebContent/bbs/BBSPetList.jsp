				...
				<div class="card">
					...
					<div class="container">
						<table class="table table-bordered" style="text-align:center;">
							...
							<tbody>
								<%-- 게시글 데이터 --%>
								<c:forEach var="pvo" items="${requestScope.lvop.list}" varStatus="status">
									<tr>
										<td class="text-center">${fn:length(requestScope.lvop.list)-status.index}</td>
										<td><c:choose>
					...
					<div class="container">
						<div class="row">
							<div class="pagingArea col-sm-4">
								<c:set var="pb" value="${requestScope.lvop.pagingBean}" />
								<ul class="pagination">
									<c:if test="${pb.previousPageGroup}">
										<li><a
											href="front?command=PetPostList&pageNo=${pb.startPageOfPageGroup-1}">&laquo;</a></li>
									</c:if>
									<c:forEach var="i" begin="${pb.startPageOfPageGroup}"
										end="${pb.endPageOfPageGroup}">
										<c:choose>
											<c:when test="${empty requestScope.lvop.list}"></c:when>
											<c:when test="${pb.nowPage!=i}">
												<li><a href="front?command=PetPostList&pageNo=${i}">${i}</a></li>
											</c:when>
											<c:otherwise>
									...
									<c:if test="${pb.nextPageGroup}">
										<li><a
											href="front?command=PetPostList&pageNo=${pb.endPageOfPageGroup+1}">&raquo;</a>
										</li>
