					...
					<div class="container">
						<div class="row">
							<div class="pagingArea col-sm-4">
								<c:set var="pb" value="${requestScope.lvoo.pagingBean}" />
								<ul class="pagination">
									<c:if test="${pb.previousPageGroup}">
										<li><a
											href="front?command=OldPostList&pageNo=${pb.startPageOfPageGroup-1}">&laquo;</a></li>
									</c:if>
									<c:forEach var="i" begin="${pb.startPageOfPageGroup}"
										end="${pb.endPageOfPageGroup}">
										<c:choose>
											<c:when test="${empty requestScope.lvoo.list}"></c:when>
											<c:when test="${pb.nowPage!=i}">
												<li><a href="front?command=OldPostList&pageNo=${i}">${i}</a></li>
											</c:when>
											<c:otherwise>
									...
									<c:if test="${pb.nextPageGroup}">
										<li><a
											href="front?command=OldPostList&pageNo=${pb.endPageOfPageGroup+1}">&raquo;</a>
										</li>
